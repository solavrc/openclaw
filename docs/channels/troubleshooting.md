...
| Typing/token usage but no Discord message | Check whether this is an ambient room event, an opted-in `message_tool` room where the model missed `message(action=send)`, or a pending durable delivery recovery entry | Inspect the gateway verbose log for suppressed final payload metadata, delivery recovery messages such as `refusing blind replay without adapter reconciliation`, verify `messages.groupChat.unmentionedInbound`, read [Ambient room events](/channels/ambient-room-events), or keep `messages.groupChat.visibleReplies: "automatic"` for normal group requests. |
